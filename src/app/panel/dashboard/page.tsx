...
import { formatBRL, getMonthKey, getMonthLabel, categoryEmoji, PREDEFINED_CATEGORIES, calcBalance } from '@/lib/utils';
...
export default function DashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {accounts.filter(a => !a.parentAccountId).length > 0 && (
        <div className="flex gap-3 overflow-x-auto p-1 pb-3" style={{ scrollbarWidth: 'thin', scrollSnapType: 'x mandatory', scrollbarColor: 'var(--border) transparent' }}>
          {accounts.filter(a => !a.parentAccountId).map(acc => {
            const vaults = accounts.filter(v => v.parentAccountId === acc.id);
            return (
              <div
                key={acc.id}
                className="flex flex-1 shrink-0 basis-[240px] flex-col gap-1 rounded-[var(--radius)] border px-5 py-4 transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow)]"
                style={{
              ...
              >
                ...
                <span className="mb-1 text-[20px] font-bold tracking-tight font-mono" style={{ color: acc.color }}>{formatBRL(calcBalance(acc.id, acc, transactions))}</span>

                {vaults.length > 0 ? (
                  <div className="mt-2 flex flex-col gap-1.5 border-t border-[var(--border)] pt-2">
                    {vaults.map(v => (
                      <div key={v.id} className="flex gap-2 items-center justify-between text-xs">
                        <span className="text-[var(--text-3)] text-nowrap flex items-center gap-1.5">
                          <i className="bi bi-safe2" style={{ color: v.color }} /> {v.name}
                        </span>
                        <span className="font-mono font-medium text-[var(--text-2)]">{formatBRL(calcBalance(v.id, v, transactions))}</span>
                      </div>
                    ))}
                  </div>
                ) : (
